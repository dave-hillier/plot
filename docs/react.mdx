...
Replot provides a native React component API. You compose charts declaratively with JSX using the `<Replot>` root component and mark components like `<Dot>`, `<Line>`, `<BarY>`, and others. An imperative API (`import * as Replot from "replot"`) also exists for non-React environments; see [Getting started](./getting-started.md).
