import {AreaY, LineY, Plot, RuleY, map, windowY} from "replot/react";
...
For example, the band chart below shows the daily high and low temperature in San Francisco. The <span style={{borderBottom: "solid 2px var(--vp-c-red)"}}>red</span> line represents the {k}-day average high, and the <span style={{borderBottom: "solid 2px var(--vp-c-blue)"}}>blue</span> line the {k}-day average low.
...
<Plot y={{
  ...
  <LineY data={sftemp} {...windowY(k, {x: "date", y: "low", stroke: "blue"})} />
  <LineY data={sftemp} {...windowY(k, {x: "date", y: "high", stroke: "red"})} />
</Plot>
...
<Plot y={{
  ...
  <LineY data={sftemp} {...windowY({k: 28, anchor}, {x: "date", y: "high"})} />
</Plot>
...
<Plot y={{
  ...
  <LineY data={sftemp} {...windowY({k: 28, anchor: "end", strict}, {x: "date", y: "low"})} />
</Plot>
...
<Plot y={{grid: true}} color={{scheme: "BuYlRd", domain: [-0.5, 0.5]}}>
  <RuleY data={[0]} />
  <LineY data={bls} {...map(
...
## window(*k*) <VersionBadge version="0.2.3" />
```js
