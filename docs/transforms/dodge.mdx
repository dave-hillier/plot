import {BinX, Dot, DotX, DodgeX, DodgeY, Replot, RectY, RuleX, RuleY, Text} from "replot/react";
...
<Replot insetRight={10} height={790}>
  <DodgeY r="rMVOP">
    <Dot data={ipos} x="date" title={(d) => `${d.NAME}\n${(d.rMVOP / 1e3).toFixed(1)}B`} fill="currentColor" />
  </DodgeY>
  <DodgeY r="rMVOP">
    <Text data={ipos} filter={(d) => d.rMVOP > 5e3} x="date" text={(d) => (d.rMVOP / 1e3).toFixed()} fill="var(--vp-c-bg)" pointerEvents="none" />
  </DodgeY>
</Replot>
...
<Replot insetRight={10} height={790}>
  <DodgeY r="rMVOP">
    <Text data={ipos} x="date" text={(d) => (d.rMVOP / 1e3).toFixed(1)} title="NAME" fontSize={(d) => Math.min(22, Math.cbrt(d.rMVOP / 1e3) * 6)} />
  </DodgeY>
</Replot>
