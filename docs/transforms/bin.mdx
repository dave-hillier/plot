import {AreaY, Bin, BinX, BinY, Dot, LineY, Plot, Rect, RectX, RectY, RuleX, RuleY, binX, scale} from "replot/react";
...
<Plot y={{grid: true}}>
  <BinX y="count" filter={null}>
    <AreaY data={olympians} x="weight" fillOpacity={0.2} />
    <LineY data={olympians} x="weight" />
  </BinX>
  <RuleY data={[0]} />
</Plot>
...
<Plot marginLeft={100} padding={0} x={{grid: true}} fy={{domain: d3.groupSort(olympians.filter((d) => d.weight), (g) => d3.median(g, (d) => d.weight), (d) => d.sport)}} color={{scheme: "turbo"}}>
  <BinX fill="proportion-facet">
    <Rect data={olympians} x="weight" fy="sport" inset={0.5} />
  </BinX>
</Plot>
