import {AreaY, Bin, BinX, BinY, Dot, LineY, Replot, Rect, RectX, RectY, RuleX, RuleY, binX, scale} from "replot/react";
...
<Replot marginLeft={100} padding={0} x={{grid: true}} fy={{domain: d3.groupSort(useOlympians().filter((d) => d.weight), (g) => d3.median(g, (d) => d.weight), (d) => d.sport)}} color={{scheme: "turbo"}}>
