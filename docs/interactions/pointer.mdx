import {Dot, LineY, Plot, RectY, RuleX, RuleY, Text, binX, stackY} from "replot/react";
...
export const pointered = true;
export const setPointered = () => {};
export const linetip = "xy";
export const setLinetip = () => {};
export const recttip = "x";
export const setRecttip = () => {};
...
<Plot>
  <Dot data={penguins} {...(pointered ? Plot.pointer : (o) => o)({x: "culmen_length_mm", y: "culmen_depth_mm", fill: "red", r: 8})} />
...
{/* VitePress v-else block removed - not applicable in React/MDX */}
...
<Plot x={{label: "Daily volume (log₁₀)"}}>
  <RectY data={aapl} {...binX({y: "count"}, {x: (d) => Math.log10(d.Volume), thresholds: 40, tip: recttip})} />
  <Dot data={aapl} {...stackY(Plot.binX({y: "count"}, {x: (d) => Math.log10(d.Volume), thresholds: 40, stroke: "red"}))} />
</Plot>
...
<Plot height={160} y={{axis: "right", grid: true, nice: true}}>
  <LineY data={aapl} x="Date" y="Close" />
  <RuleX data={aapl} {...Plot.pointerX({x: "Date", py: "Close", stroke: "red"})} />
  <Dot data={aapl} {...Plot.pointerX({x: "Date", y: "Close", stroke: "red"})} />
  <Text data={aapl} {...Plot.pointerX({px: "Date", py: "Close", dy: -17, frameAnchor: "top-left", fontVariant: "tabular-nums", text: (d) => [`Date ${Plot.formatIsoDate(d.Date)}`, `Close ${d.Close.toFixed(2)}`].join("   ")})} />
</Plot>
