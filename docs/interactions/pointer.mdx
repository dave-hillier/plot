import {BinX, Dot, formatIsoDate, LineY, Replot, pointer, pointerX, RectY, RuleX, RuleY, StackY, Text} from "replot/react";
...
Replot.plot({
  marks: [
    Replot.dot(penguins, Replot.pointer({x: "culmen_length_mm", y: "culmen_depth_mm", fill: "red", r: 8})),
    Replot.dot(penguins, {x: "culmen_length_mm", y: "culmen_depth_mm"})
  ]
})
```
...
With the imperative API, the same event is available on the rendered plot element, which also allows using a plot as an [Observable view](https://observablehq.com/@observablehq/views) (viewof):

```js
const plot = Replot.plot(options);
