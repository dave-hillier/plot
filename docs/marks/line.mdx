...
{/*
// TODO: manually convert: d3.text("../data/beagle.csv").then((text) => (beagle.value = d3.csvParseRows(text).map(d3.autoType)));
// TODO: manually convert: d3.csv("../data/us-population-state-age.csv", d3.autoType).then((data) => {
// TODO: manually convert: const ages = data.columns.slice(1); // convert wide data to tidy data
// TODO: manually convert: stateage.value = Object.assign(ages.flatMap((age) => data.map((d) => ({state: d.name, age, population: d[age]}))), {ages});
// TODO: manually convert: Promise.all([
// TODO: manually convert: d3.csv("../data/amzn.csv", d3.autoType),
// TODO: manually convert: d3.csv("../data/goog.csv", d3.autoType),
// TODO: manually convert: d3.csv("../data/ibm.csv", d3.autoType)
// TODO: manually convert: ]).then((datas) => {
// TODO: manually convert: stocks.value = d3.zip(["AAPL", "AMZN", "GOOG", "IBM"], [aapl].concat(datas)).flatMap(([Symbol, data]) => data.map((d) => ({Symbol, ...d})));
*/}
...
{/* Plot.lineY(d3.shuffler(d3.randomLcg(42))(aapl.slice()), {x: "Date", y: "Close"}).plot() // 🌶️ */}
...
<Plot y={{
  ...
  <Text data={stocks} {...selectLast(Plot.normalizeY({
      x: "Date",
      y: "Close",
      z: "Symbol",
      text: "Symbol",
...
<Plot projection="equirectangular">
  <Geo data={land} />
  {/* TODO: // MultiPolygon
    Plot.line(beagle, {stroke: "red"}) */}
  {/* TODO: // [[lon, lat], …]
    Plot.geo({type: "Point", coordinates: [-0.13, 51.5]}, {fill: "red"}) */}
</Plot>
