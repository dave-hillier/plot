import {Centroid, Geo, GeoCentroid, Plot, Spike, Vector} from "replot/react";
...
<Plot inset={10} aspectRatio={1} color={{label: "Speed (m/s)", zero: true, legend: true}}>
  <Vector data={wind} x="longitude" y="latitude" rotate={({u, v}) => Math.atan2(u, v) * 180 / Math.PI} length={({u, v}) => Math.hypot(u, v)} stroke={({u, v}) => Math.hypot(u, v)} />
</Plot>
...
<Plot projection="albers-usa" length={{type: "sqrt", transform: Math.abs}}>
  <Geo data={statemesh} strokeWidth={0.5} />
  <Geo data={nation} />
  <Centroid>
    <Vector data={counties} anchor="start" length={(d) => d.properties.margin2020 * d.properties.votes} stroke={(d) => d.properties.margin2020 > 0 ? "red" : "blue"} rotate={(d) => d.properties.margin2020 > 0 ? 60 : -60} />
  </Centroid>
</Plot>
...
<Plot width={688} projection="albers-usa" length={{range: [0, 200]}}>
  <Geo data={statemesh} strokeOpacity={0.5} />
  <Geo data={nation} />
  <GeoCentroid>
    <Spike data={counties} length={(d) => d.properties.population} stroke="green" />
  </GeoCentroid>
</Plot>
...
<Plot inset={6} width={1024} aspectRatio={1} axis={null}>
...
## spike(*data*, *options*) <VersionBadge version="0.6.2" />
```js
