import {BinX, Dot, Frame, Geo, GeoCentroid, LineY, Plot, pointer, RectY, Tip} from "replot/react";
...
<Plot>
  <BinX y="count">
    <RectY data={olympians} x="weight" fill="sex" tip />
  </BinX>
</Plot>
...
<Plot y={{label: "Frequency"}}>
  <BinX y="sum">
    <RectY data={olympians} x="weight" y={(d) => d.sex === "male" ? 1 : -1} fill="sex" tip />
  </BinX>
</Plot>
...
<Plot projection="albers-usa">
  <Geo data={states} />
  <GeoCentroid>
    <Tip data={states} title={(d) => d.properties.name} anchor="bottom" textPadding={3} />
  </GeoCentroid>
</Plot>
