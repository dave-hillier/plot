...
{(() => {
  ...
  return (
    <Plot height={720} axis={null} y={{domain: [0, step]}} color={{scheme: "YlGnBu"}} facet={{data: industries, y: "industry"}}>
      {d3.range(bands).map((i) => <AreaY key={i} data={industries} x="date" y={(d) => d.unemployed - i * step} fill={i} clip />)}
      <Text data={industries} {...selectFirst({text: "industry", frameAnchor: "top-left", dx: 6, dy: 6})} />
      <Frame />
    </Plot>
  );
})()}
