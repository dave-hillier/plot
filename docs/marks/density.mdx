import {Density, Dot, Frame, Geo, Replot} from "replot/react";
...
<Replot projection="albers" color={{scheme: "turbo"}}>
...
Replot.plot({
  axis: null,
  marks: [
    Replot.dot(penguins, {x: "flipper_length_mm", y: "culmen_length_mm"}),
    Replot.density(penguins, {x: "flipper_length_mm", y: "culmen_length_mm"})
  ]
})
``` */}
