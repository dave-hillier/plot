import {AxisFy, Dot, Geo, Graticule, Plot, Sphere, centroid} from "replot/react";
...
// TODO: manually convert: d3.csv("../data/us-county-unemployment.csv")
// TODO: manually convert: ]).then(([_us, _unemployment]) => {
// TODO: manually convert: const map = new Map(_unemployment.map((d) => [d.id, +d.rate]));
// TODO: manually convert: _us.objects.counties.geometries.forEach((g) => (g.properties.unemployment = map.get(g.id)));
...
<Plot projection="albers-usa" color={{
    type: "quantile",
    n: 9,
    scheme: "blues",
    label: "Unemployment (%)",
  ...
  <Geo data={counties} fill="unemployment" title={(d) => `${d.properties.name} ${d.properties.unemployment}%`} tip />
</Plot>
...
<Plot projection="equirectangular" r={{transform: (r) => Math.pow(10, r)}}>
  <Geo data={land} fill="currentColor" fillOpacity={0.2} />
  <Sphere />
  <Geo data={earthquakes} r="mag" fill="red" fillOpacity={0.2} stroke="red" title="title" href="url" target="_blank" />
</Plot>
...
<Plot projection="albers-usa">
  <Geo data={states} strokeOpacity={0.1} tip title="name" />
  <Geo data={nation} />
...
<Plot margin={0} padding={0} projection="albers" fy={{interval: "10 years"}}>
  <Geo data={statemesh} strokeOpacity={0.2} />
  <Geo data={nation} />
  <Geo data={walmarts} fy="date" r={1.5} fill="blue" tip title="date" />
...
## sphere(*options*) <VersionBadge version="0.6.1" />
```js
Plot.sphere()
```
...
## graticule(*options*) <VersionBadge version="0.6.1" />
```js
Plot.graticule()
```
