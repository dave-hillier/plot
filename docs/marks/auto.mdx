import {Auto, Replot} from "replot/react";
...
Returns an auto mark *options* object with no option undefined; the mark type, reducers, and other options are all populated. The `autoSpec` function is exported from `replot/react`.
