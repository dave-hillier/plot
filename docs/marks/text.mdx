import {BarY, Frame, Line, Plot, RuleX, RuleY, Text, selectFirst, stackX2} from "replot/react";
...
<Plot y={{
  ...
  <Text data={travelers} {...selectFirst({x: "date", y: "previous", text: ["2019"], fillOpacity: 0.5, lineAnchor: "bottom", dy: -6})} />
  <Text data={travelers} {...selectFirst({x: "date", y: "current", text: ["2020"], lineAnchor: "top", dy: 6})} />
</Plot>
...
<Plot width={240} axis={null} x={{type: "point"}} y={{type: "point", domain: d3.range(4, 25)}} color={{domain: "NLB", range: ["currentColor", "peru", "brown"], legend: true}}>
  <Text data={[[0.5, 4]]} text={["Northbound"]} textAnchor="start" dx={16} />
  <Text data={[[-0.5, 4]]} text={["Southbound"]} textAnchor="end" dx={-16} />
  <Text data={d3.range(5, 25)} x={0} y={Plot.identity} text={(y) => `${y % 12 || 12}${y % 24 >= 12 ? "p": "a"}`} />
  <Text data={caltrain} {...stackX2({x: (d) => d.orientation === "N" ? 1 : -1, y: "hours", fill: "type", text: "minutes"})} />
  <RuleX data={[-0.5, 0.5]} />
</Plot>
...
<Plot aspectRatio={1} inset={10} grid>
...
<Plot height={320} x={{type: "point", align: 0, axis: "top", tickSize: 0}}>
  <Text data={[
