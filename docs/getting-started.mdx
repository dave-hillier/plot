...
A quick way to try the Plot API online is on [Observable](https://observablehq.com), where **Observable Plot** — the library Replot is forked from — is available by default in notebooks as part of Observable’s standard library. Replot’s imperative API is compatible, so code you sketch there carries over. To use Plot in a notebook, simply return the generated plot from a cell like so:
...
```
See the upstream [Plot + Vue CodeSandbox](https://codesandbox.io/p/sandbox/plot-vue-jlgg2w?file=/src/App.vue) for details.
...
```
See the upstream [Plot + Svelte REPL](https://svelte.dev/playground/e65b5c87ae7e44239cef41ec3df28f52?version=5.2.7) for details.
