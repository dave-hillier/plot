import {BarY, Dot, Frame, LineY, Replot, RuleY, Text} from "replot/react";
...
To render a **plot** in Replot, use the `<Replot>` component from `replot/react`, composing [marks](./marks.md) as children and passing the desired *options* as props. (The equivalent imperative entry point is [plot](#plot), typically called as `Replot.plot`, which returns an SVG or HTML figure element.)
...
By default, [plot](#plot) returns an SVG element; however, if the plot includes a title, subtitle, [legend](./legends.md), or caption, plot wraps the SVG element with an HTML figure element. You can also force Replot to generate a figure element by setTing the **figure** option <VersionBadge version="0.6.10" pr="1761" /> to true.
...
This is the imperative entry point, equivalent to the `<Replot>` component above; use it outside of React.

```js
Replot.plot({
  height: 200,
  marks: [
    Replot.barY(alphabet, {x: "letter", y: "frequency"})
  ]
})
```

Renders a new plot with the specified *options*, returning a SVG or HTML figure element. This element can then be inserted into the page as described in the [getting started guide](../getting-started.md). In React, prefer the `<Replot>` component, which renders into the React tree.

## *mark*.plot(*options*) {/* {#mark_plot} */}

This shorthand is part of the imperative API only.

```js
Replot.barY(alphabet, {x: "letter", y: "frequency"}).plot({height: 200})
```
...
This method is part of the imperative API: it operates on a rendered plot element returned by [plot](#plot).

```js
const plot = Replot.plot(options); // render a plot
...
This method is likewise imperative-only; in React, use the `<Legend>` component or the scale **legend** option instead.
