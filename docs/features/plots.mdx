import {BarY, Dot, Frame, LineY, Plot, RuleY, Text} from "replot/react";
...
export function MarginDemo() {
  ...
  return (
    <div>
      ...
      <Plot marginTop={marginTop} marginRight={marginRight} marginBottom={marginBottom} marginLeft={marginLeft} grid>
...
export function AspectRatioDemo() {
  ...
  return (
    <div>
      ...
      <Plot grid inset={10} aspectRatio={fixed ? 1 : undefined} color={{legend: true}}>
        <Frame />
...
```jsx
<Plot marginTop={20} marginRight={20} marginBottom={30} marginLeft={40} grid>
  <BarY data={alphabet} x="letter" y="frequency" fill="green" />
  <Frame />
</Plot>
```
...
<Plot title="For charts, an informative title" subtitle="Subtitle to follow with additional context" caption="Figure 1. A chart with a title, subtitle, and caption.">
  <Frame />
