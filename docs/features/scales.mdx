...
<Plot x={{domain: [0, 100], reverse: true, grid: true}} />
...
<Plot x={{type: "utc", domain: [1609459200000, 1640995200000], grid: true}} />

If the scale **type** is *time*, the ticks will be in local time — as with the dates below — rather than UTC.

<Plot x={{type: "time", domain: [new Date(2021, 0, 1), new Date(2022, 0, 1)], grid: true}} />
...
<Plot x={{type: "log", domain: [1e0, 1e5], grid: true}} />
...
<Plot x={{type: "log", domain: [1e0, 1e5], tickFormat: ",", grid: true}} />
...
<Plot x={{type: "log", base: 2, domain: [1e0, 1e4], ticks: 20, grid: true}} />
...
<Plot x={{type: "symlog", domain: [-10, 10], grid: true}} />
...
<Plot x={{type: "sqrt", domain: [0, 100], grid: true}} />

<Plot x={{type: "pow", exponent: 1 / 3, domain: [0, 100], grid: true}} />
...
<Plot x={{type: "point", domain: "ABCDEFGHIJ", grid: true}} />
...
```jsx{1}
<Plot y={{percent: true}} color={{scheme: "BuRd"}}>
...
<Plot x={{domain: [new Date("1880-01-01"), new Date("2016-11-01")]}}>
  <LineY data={gistemp} x="Date" y="Anomaly" />
</Plot>
