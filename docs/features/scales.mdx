import {BarY, Cell, CellX, Dot, Frame, LineY, Replot, RuleX, RuleY, TickX, identity, scale} from "replot/react";
...
As another example, with the imperative API, [*plot*.scale](./plots.md#plot_scale) lets a second plot reuse the *color* scale from a first, already-rendered plot:

```js
const plot1 = Replot.plot({...options1});
const plot2 = Replot.plot({...options2, color: plot1.scale("color")});
```
