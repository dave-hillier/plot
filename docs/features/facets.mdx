import {AreaY, Arrow, BarX, BoxX, Dot, Frame, Line, Plot, Text, groupY, normalizeY} from "replot/react";
...
<Plot height={800} marginRight={90} marginLeft={110} grid x={{nice: true}} y={{inset: 5}} color={{type: "categorical"}}>
  <Frame />
...
<Plot height={800} marginLeft={110} grid x={{nice: true}} y={{inset: 5}} color={{scheme: "spectral", label: "Change in yield", tickFormat: "+f", legend: true}} facet={{marginRight: 90}}>
  <Frame />
  <Arrow data={barley} {...groupY({
      x1: "first",
      x2: "last",
...
<Plot grid marginRight={60} facet={{label: null}}>
  <Frame />
...
{/* TODO: manually convert this facet-wrapping plot
  Uses IIFE to compute fx/fy from industry index for wrapping facets into columns.
  Needs: industries data, normalizeY transform, AreaY, Text, Frame marks.
*/}
...
<Plot marginLeft={60} marginRight={60} grid y={{label: null}} fy={{label: null}} color={{legend: true}}>
  <BarX data={penguins} {...groupY({x: "count"}, {fy: "species", y: "island", fill: "sex"})} />
...
<Plot grid aspectRatio={0.5} facet={{data: anscombe, x: "series"}}>
  <Frame />
