import {AreaY, Arrow, BarX, BoxX, Dot, Frame, Line, Plot, Text, groupY, normalizeY, scale} from "replot/react";
...
{(() => {
  ...
  return (
    <Plot height={industryNames.length / columns * 120} axis={null} y={{insetTop: 4}} fx={{padding: 0.03}} fy={{padding: 0.03}}>
      <AreaY data={industries} {...normalizeY("extent", {
        x: "date",
        y: "unemployment",
      ...
      <Text data={industries} {...normalizeY("extent", {
        x: "date",
        y: "unemployment",
        fy: (d) => Math.floor(industryNames.indexOf(d.industry) / columns),
        fx: (d) => industryNames.indexOf(d.industry) % columns,
