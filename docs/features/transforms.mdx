import {Cell, Plot, RectY, RuleX, TickX, binX, group, groupY, normalizeY, identity, valueof, column, indexOf} from "replot/react";
...
<Plot marginLeft={120} x={{label: "Vehicles per hour (thousands)", transform: (x) => x / 1000}} y={{label: null}}>
  <RuleX data={[0]} />
  <TickX data={traffic} x="vehicles" y="location" strokeOpacity={0.3} />
  <TickX data={traffic} {...groupY(
        {x: "median"},
...
<Plot marginLeft={120} padding={0} y={{label: null}} color={{scheme: "turbo", legend: true, zero: true}}>
  <Cell data={traffic} {...group(
        {fill: "median"},
...
<Plot>
  <RectY data={bins} x1="x0" x2="x1" y2="length" />
</Plot>
...
Transforms are composable: you can pass *options* through more than one transform before passing it to a mark. For example, above it’s a bit difficult to compare the weight distribution by sex because there are fewer <span style={{borderBottom: "solid 2px #4269d0"}}>female</span> than <span style={{borderBottom: "solid 2px #efb118"}}>male</span> athletes in the data. We can remove this effect using the [normalize transform](../transforms/normalize.md) with the *sum* reducer.

<Plot y={{percent: true}}>
  <RectY data={olympians} {...normalizeY(
...
## transform(*options*, *transform*) <VersionBadge version="0.4.3" />
```js
...
## initializer(*options*, *initializer*) <VersionBadge version="0.5.0" />
...
## column(*source*) <VersionBadge version="0.4.3" />
```js
...
## identity <VersionBadge version="0.6.2" />
```js
...
## indexOf <VersionBadge version="0.6.6" />
```js
