import {AreaY, BarX, BarY, Dot, LineY, Replot, RuleY} from "replot/react";
...
Every mark is available both as a [React component](../getting-started.md#plot-in-react) (e.g., `<Dot data={data} x="x" y="y" />`) and as an imperative function (e.g., `Replot.dot(data, options)`). The examples on this page use the React components, but all the same options and behaviors apply to the imperative API.
...
The imperative API also supports inserting arbitrary SVG elements (such as gradients) as functions in the marks array. Here is the equivalent gradient example using the imperative API:
...
Replot.plot({
  marks: [
    ...
    Replot.barY(alphabet, {x: "letter", y: "frequency", fill: "url(#gradient)"}),
    Replot.ruleY([0])
  ]
})
```
...
<Replot>
  <LineY data={bls} x="date" y="unemployment" z="division" />
</Replot>
...
This helper is part of the imperative API; in React, compose marks by nesting components inside `<Replot>` instead.

```js
Replot.marks(
  Replot.ruleY([0]),
  Replot.areaY(data, {fill: color, fillOpacity, ...options}),
  Replot.lineY(data, {stroke: color, ...options})
)
```
