import {AreaY, BarX, BarY, Dot, LineY, Plot, RuleY} from "replot/react";
...
The rectY mark with interval requires the transform-based interval support which is not yet available in the React API. Here is the imperative equivalent:

```js
Plot.rectY(timeseries, {x: "year", y: "population", interval: 1}).plot({x: {tickFormat: ""}})
```

Alternatively, you can keep the barY mark and apply the **interval** option to the *x* scale.

```js
Plot.barY(timeseries, {x: "year", y: "population"}).plot({x: {tickFormat: "", interval: 1}})
```
...
## marks(...*marks*) <VersionBadge version="0.2.0" />
```js
Plot.marks(
  Plot.ruleY([0]),
