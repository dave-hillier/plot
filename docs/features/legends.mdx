import {Dot, Replot, RuleY} from "replot/react";
...
```js
Replot.legend({color: {type: "linear"}})
```
...
<Replot width={320} color={{type: "linear", legend: true}} />

```js
Replot.legend({width: 320, color: {type: "linear"}})
```
